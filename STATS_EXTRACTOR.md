...
This fork adds minimal modifications to PlantUML (version 1.2026.2beta3) to expose internal data structures required for statistics extraction. No changes were made to PlantUML's parsing logic or diagram rendering.

## Changes from Upstream PlantUML

- **9 public getter methods** added to activity diagram instruction classes to expose private fields for tree traversal (41 lines across 9 files)
...
This produces a single executable JAR at `build/libs/plantuml-1.2026.2beta3.jar`.

## Usage

```bash
# Single file
java -cp build/libs/plantuml-1.2026.2beta3.jar \
